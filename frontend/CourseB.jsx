...
function CourseB() {
  ...
  const fetchQueue = async () => {
    const response = await fetch("http://127.0.0.1:5000/get_queue/Course B");
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("http://127.0.0.1:5000/join_queue", {
      ...
      body: JSON.stringify({ name, email, course: "Course B", group_size: groupSize }),
    });
